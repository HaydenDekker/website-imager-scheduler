...
public class DeviceFlowSubscriber implements DeviceFlowSubscriberPort{
	
	Logger log = LoggerFactory.getLogger(DeviceFlowSubscriber.class);
		...
		fromDevice(device)
			.subscribe(c-> {
				c.ifPresentOrElse(af->{
					...
					subscriptionSink.tryEmitError(
							new Exception("couldn't find app flow for " + device.getName()));
				});	
			}
		);
	...
	private Mono<Optional<AppFlow>> fromDevice(Device device) {
		return deviceFlowAssignmentSupplier.find(device)
				.doOnNext(c-> c.ifPresentOrElse(
						d -> log.info("Finding flow for " + device.getName()), 
						()->log.error("Couldn't find device.")))
							//return Mono.error(new Exception("Could not find device with id " + device.getId()));}))
			.map(o->
				o.map(ass->{
